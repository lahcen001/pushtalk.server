...
io.use((socket, next) => {
  const apiSecret = socket.handshake.auth.apiSecret;
  
  if (!apiSecret) {
    console.log('❌ Connection rejected: No API secret provided');
    return next(new Error('Authentication required'));
  }
  
  if (apiSecret !== API_SECRET) {
    console.log('❌ Connection rejected: Invalid API secret');
...
io.on('connection', (socket) => {
  ...
  // PTT events with floor control
  socket.on('ptt:down', ({ roomId }) => {
    const room = rooms.get(roomId);
    if (!room) return;

    // Check if someone else has the floor
    if (room.currentSpeaker && room.currentSpeaker !== socket.id) {
      console.log(`⛔ Floor busy: ${socket.id} tried to talk while ${room.currentSpeaker} is speaking`);
      socket.emit('floor_busy', { 
        currentSpeaker: room.currentSpeaker,
        message: 'Someone else is talking. Please wait.'
      });
      return;
    }

    // Grant floor to this user
    room.currentSpeaker = socket.id;
    console.log(`🎤 Floor granted to: ${socket.id}`);

    // Clear any existing floor timeout
    if (room.floorTimeout) {
      clearTimeout(room.floorTimeout);
    }

    // Auto-release floor after 30 seconds
    room.floorTimeout = setTimeout(() => {
      if (room.currentSpeaker === socket.id) {
        console.log(`⏱️ Auto-releasing floor for: ${socket.id}`);
        room.currentSpeaker = null;
        room.floorTimeout = null;
        
        // Notify all participants that floor is released
        io.to(roomId).emit('participant_speaking', {
          userId: socket.id,
          speaking: false,
        });
        io.to(roomId).emit('floor_released', { userId: socket.id });
      }
    }, 30000);
    ...
    socket.to(roomId).emit('floor_taken', { userId: socket.id });
  });
  ...
  socket.on('ptt:up', ({ roomId }) => {
    ...
    if (room.currentSpeaker !== socket.id) {
      console.log(`⚠️ ${socket.id} tried to release floor but doesn't have it`);
      return;
    }

    // Release the floor
    room.currentSpeaker = null;
    console.log(`🎤 Floor released by: ${socket.id}`);
